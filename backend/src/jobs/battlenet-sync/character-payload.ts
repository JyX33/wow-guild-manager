import { DbCharacter, DbGuild, EnhancedCharacterData } from '../../../../shared/types/guild.js';
  ...
  if (bnetGuildId && bnetGuildData) {
    ...
    try {
      ...
      if (localGuild) {
        ...
        await queueMissingGuildSync(guildModel, { id: bnetGuildId, name: bnetGuildData.name, realm: { slug: enhancedData.realm.slug, name: fullRealmName } }, character.region as BattleNetRegion);
...
async function queueMissingGuildSync(
  ...
  try {
    const existingByBNetId = await guildModel.findOne({ bnet_guild_id: bnetGuildId });
    if (existingByBNetId) {
      logger.info(logContext, `${logPrefix} Guild already exists locally (found by BNet ID). Skipping queue.`);
      return;
    }

    const existingByName = await guildModel.findOne({ name: guildName, realm: realmName, region: region });
    if (existingByName) {
      ...
      if (!existingByName.bnet_guild_id) {
        try {
          await guildModel.update(existingByName.id, { bnet_guild_id: bnetGuildId });
