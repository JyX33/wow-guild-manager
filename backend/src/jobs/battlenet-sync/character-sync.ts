...
export async function syncCharacter(
  ...
  try {
    ...
    const enhancedDataResult = await apiClient.getEnhancedCharacterData(realmSlug, characterNameLower, region);
    ...
    if (enhancedDataResult === null) {
      ...
      try {
        ...
        try {
          const membersToMark = await guildMemberModel.findAll({ character_id: character.id });
    ...
    if (localGuild) {
      ...
      if (currentMembership) {
        const memberUpdatePayload: Partial<DbGuildMember> & { is_available: boolean } = {
          character_name: finalUpdatePayload.name || character.name,
          character_class: finalUpdatePayload.class || character.class,
          is_available: true,
        };
        await guildMemberModel.update(currentMembership.id, memberUpdatePayload);
