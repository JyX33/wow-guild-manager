...
export async function calculateCharacterToyHash(
  ...
  if (character.user_id !== null) {
    logger.trace(
      { charId: character.id, charName: character.name },
      `[SyncService][ToyHash] Skipping calculation for linked character.`,
    );
    return null;
  }
  if (!character.name || !character.realm || !character.region) {
    logger.warn(
      ...
      "[SyncService][ToyHash] Cannot calculate: Missing character name, realm, or region.",
    );
    return null;
  }
  ...
  logger.debug(
    logContext,
    `[SyncService][ToyHash] Calculating for unlinked character ${character.name}.`,
  );

  try {
    logger.trace(
      logContext,
      `[SyncService][ToyHash] Fetching collections index.`,
    );
    const collectionsIndex = await apiClient.getCharacterCollectionsIndex(
      realmSlug,
      characterNameLower,
      region,
    );
    ...
    if (!collectionsIndex?.toys?.href) {
      logger.debug(
        logContext,
        `[SyncService][ToyHash] Toys collection href not found in index. Using NO_TOYS_HASH.`,
      );
      return NO_TOYS_HASH;
    }
    ...
    if (
      ...
      if (toyIds.length === 0) {
        logger.debug(
          logContext,
          `[SyncService][ToyHash] No valid toy IDs found in fetched data. Using NO_TOYS_HASH.`,
        );
        return NO_TOYS_HASH;
      }

      const toyIdString = toyIds.join(",");
      const calculatedHash = crypto.createHash("sha256").update(toyIdString)
        .digest("hex");
      logger.debug({
        ...logContext,
        toyCount: toyIds.length,
        hash: calculatedHash,
      }, `[SyncService][ToyHash] Calculated hash.`);
      return calculatedHash;
    } else {
      logger.debug(
        logContext,
        `[SyncService][ToyHash] No toys found or invalid format in fetched data. Using NO_TOYS_HASH.`,
      );
      return NO_TOYS_HASH;
    }
  } catch (error: unknown) {
    if (axios.isAxiosError(error) && error.response?.status === 404) {
      logger.warn(
        logContext,
        `[SyncService][ToyHash] Collections index or toys endpoint returned 404. Assuming no toys/profile issue. Using NO_TOYS_HASH.`,
      );
      return NO_TOYS_HASH;
    } else {
      logger.error(
        { err: error, ...logContext },
        `[SyncService][ToyHash] Error calculating toy hash. Returning null.`,
      );
      return null;
    }
  }
