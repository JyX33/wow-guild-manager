...
export async function updateCoreGuildData(
  ...
  logger.debug(
    logContext,
    `[SyncService] Preparing to update core data for guild.`,
  );
  ...
  const guildMasterMember = bnetGuildRoster.members.find((
    m: BattleNetGuildMember,
  ) => m.rank === 0);
  ...
  if (guildMasterMember) {
    ...
    logger.debug(
      gmLogContext,
      `[SyncService] Found potential GM in roster: ${gmName}-${gmRealmSlug}. Attempting to link user.`,
    );
    ...
    try {
      ...
      if (guildMasterUser) {
        ...
        logger.info(
          { ...gmLogContext, userId: guildMasterUser.id },
          `[SyncService] Linked user ID ${guildMasterUser.id} as leader for guild.`,
        );
      } else {
        logger.info(
          gmLogContext,
          `[SyncService] User record for GM ${gmName}-${gmRealmSlug} not found locally. Guild leader remains unlinked.`,
        );
      }
    } catch (userLookupError: unknown) {
      logger.error(
        { err: userLookupError, ...gmLogContext },
        `[SyncService] Error looking up user for potential GM ${gmName}-${gmRealmSlug}. Guild leader remains unlinked.`,
      );
    }
  } else {
    logger.info(
      logContext,
      `[SyncService] No Guild Master (rank 0) found in fetched roster. Guild leader remains unlinked.`,
    );
  }
  ...
  try {
    ...
    logger.info(
      logContext,
      `[SyncService] Successfully updated core guild data.`,
    );
  } catch (updateError: unknown) {
    logger.error(
      { err: updateError, ...logContext },
      `[SyncService] Failed to update core guild data in database.`,
    );
...
> {
  ...
  logger.info(
    logContext,
    `[SyncService] Starting sync for guild: ${guildIdentifier}`,
  );
  ...
  try {
    ...
    try {
      logger.debug(
        logContext,
        `[SyncService] Fetching core guild data from Battle.net for ${guildIdentifier}.`,
      );
      bnetGuildData = await apiClient.getGuildData(
        realmSlug,
        guildNameSlug,
        region,
      );
      logger.debug(
        logContext,
        `[SyncService] Successfully fetched core guild data.`,
      );
    } catch (error: unknown) {
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        logger.warn(
          logContext,
          `[SyncService] Guild ${guildIdentifier} not found on Battle.net (404). Marking to exclude from future sync.`,
        );
      ...
      logger.error(
        ...
        `[SyncService] Failed to fetch core guild data for ${guildIdentifier}.`,
      );
    ...
    try {
      logger.debug(
        logContext,
        `[SyncService] Fetching guild roster data from Battle.net for ${guildIdentifier}.`,
      );
      bnetGuildRoster = await apiClient.getGuildRoster(
        region,
        realmSlug,
        guildNameSlug,
      );
      logger.debug(
        { ...logContext, rosterSize: bnetGuildRoster.members.length },
        `[SyncService] Successfully fetched guild roster with ${bnetGuildRoster.members.length} members.`,
      );
    } catch (error: unknown) {
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        logger.warn(
          logContext,
          `[SyncService] Guild roster for ${guildIdentifier} not found on Battle.net (404). Marking to exclude from future sync.`,
        );
        await guildModel.update(guild.id, {
          exclude_from_sync: true,
          last_updated: new Date().toISOString(),
        ...
        return {
          success: false,
          error: "Guild roster not found on Battle.net (404)",
        };
      }
      logger.error(
        { err: error, ...logContext },
        `[SyncService] Failed to fetch guild roster data for ${guildIdentifier}.`,
      );
      return { success: false, error: "Failed to fetch guild roster data" };
    }

    await updateCoreGuildData(
      guildModel,
      userModel,
      guild,
      bnetGuildData,
    ...
    logger.info(
      logContext,
      `[SyncService] Successfully completed sync cycle for guild ${guildIdentifier}.`,
    );
    return { success: true, bnetGuildData, bnetGuildRoster };
  } catch (error: unknown) {
    logger.error(
      { err: error, ...logContext },
      `[SyncService] Unhandled error during sync for guild ${guildIdentifier}. Sync cycle for this guild aborted.`,
    );
    try {
      await guildModel.update(guild.id, {
        last_updated: new Date().toISOString(),
      ...
      logger.error(
        { err: updateError, ...logContext },
        `[SyncService] Failed to update guild timestamp after unhandled sync error.`,
      );
    }
