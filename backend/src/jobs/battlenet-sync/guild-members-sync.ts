...
import {
  compareGuildMembers,
  GuildMemberComparisonRow,
} from "./guild-members-compare.js";
...
export async function syncGuildMembersTable(
  ...
  logger.info(
    logContext,
    `[SyncService] Starting guild_members table sync. Roster size: ${rosterSize}`,
  );
  ...
  try {
    await withTransaction(async (client) => {
      logger.debug(
        logContext,
        "[SyncService] Beginning transaction for guild member sync.",
      );
      ...
      existingMembersResult.rows.forEach((row: GuildMemberComparisonRow) => {
        if (row.character_name && row.realm) {
          const key =
            `${row.character_name.toLowerCase()}-${row.realm.toLowerCase()}`;
          existingMembersMap.set(key, {
            id: row.id,
          ...
          logger.warn(
            { ...logContext, memberId: row.id, characterId: row.character_id },
            `[SyncService] Skipping existing member (ID: ${row.id}) from map due to missing name or realm slug.`,
          );
        }
      });
      logger.info(
        { ...logContext, count: existingMembersMap.size },
        `[SyncService] Mapped ${existingMembersMap.size} existing members from DB.`,
      );
      ...
      bnetRoster.members.forEach((member) => {
        if (member.character?.name && member.character?.realm?.slug) {
          const key =
            `${member.character.name.toLowerCase()}-${member.character.realm.slug.toLowerCase()}`;
          rosterMembersMap.set(key, member);
        } else {
          logger.warn(
            { ...logContext, memberData: member },
            `[SyncService] Skipping BNet roster member due to missing name or realm slug.`,
          );
        }
      });
      logger.debug(
        { ...logContext, count: rosterMembersMap.size },
        `[SyncService] Mapped ${rosterMembersMap.size} members from BNet roster.`,
      );
      ...
      logger.info(
        { ...logContext, count: existingCharacterMap.size },
        `[SyncService] Mapped ${existingCharacterMap.size} existing characters matching BNet roster.`,
      );
      ...
      if (charactersToCreate.length > 0) {
        logger.debug(
          { ...logContext, count: charactersToCreate.length },
          "[SyncService] Creating new character records.",
        );
        const creationPromises = charactersToCreate.map(async (charData) => {
          try {
            const newChar = await characterModel.create(charData);
            const key =
              `${newChar.name.toLowerCase()}-${newChar.realm.toLowerCase()}`;
            ...
            logger.info(
              ...
              `[SyncService] Created character \${newChar.name}-\${newChar.realm} (ID: \${newChar.id})`,
            );
            ...
            logger.error(
              ...
              `[SyncService] Failed to create character record for \${charData.name}-\${charData.realm}.`,
            );
          }
        });
        await Promise.all(creationPromises);
        logger.info(
          { ...logContext, count: createdCharacterMap.size },
          `[SyncService] Finished creating \${createdCharacterMap.size} new characters.`,
        );
      }
      ...
      for (const charData of charactersToCreate) {
        if (!charData.name || !charData.realm) {
          logger.warn(
            { ...logContext, charData },
            `[SyncService] Skipping member creation for character data missing name/realm.`,
          );
          continue;
        }
        const key =
          `${charData.name.toLowerCase()}-${charData.realm.toLowerCase()}`;
        ...
        if (!newCharacterId) {
          logger.error(
            { ...logContext, key },
            `[SyncService] Could not find newly created character ID for key '${key}' during member creation. Skipping.`,
          );
          continue;
        }
        if (!rosterMember) {
          logger.error(
            { ...logContext, key },
            `[SyncService] Could not find BNet roster member data for key '${key}' during member creation. Skipping.`,
          );
          continue;
        }
      ...
      if (newMembersData.length > 0) {
        logger.debug(
          { ...logContext, count: newMembersData.length },
          "[SyncService] Bulk inserting new guild members.",
        );
        try {
          await guildMemberModel.bulkCreate(newMembersData);
          logger.info(
            { ...logContext, count: newMembersData.length },
            `[SyncService] Bulk inserted ${newMembersData.length} new members.`,
          );
        } catch (bulkCreateError: unknown) {
          logger.error(
            { err: bulkCreateError, ...logContext },
            `[SyncService] Error during bulk insert of new members.`,
          );
      ...
      if (membersToUpdate.length > 0) {
        logger.debug(
          { ...logContext, count: membersToUpdate.length },
          "[SyncService] Bulk updating existing guild members.",
        );
        ...
        try {
          ...
          logger.info(
            { ...logContext, count: membersToUpdate.length },
            `[SyncService] Bulk updated ${membersToUpdate.length} existing members.`,
          );
        } catch (bulkUpdateError: unknown) {
          logger.error(
            { err: bulkUpdateError, ...logContext },
            `[SyncService] Error during bulk update of existing members.`,
          );
      ...
      if (memberIdsToDeactivate.length > 0) {
        logger.debug(
          { ...logContext, count: memberIdsToDeactivate.length },
          "[SyncService] Marking members no longer in BNet roster as unavailable.",
        );
        const updatesToDeactivate = memberIdsToDeactivate.map((
          memberId: number,
        ) => ({
          memberId: memberId,
          is_available: false,
        ...
        try {
          ...
          logger.info(
            { ...logContext, count: memberIdsToDeactivate.length },
            `[SyncService] Marked ${memberIdsToDeactivate.length} members as unavailable.`,
          );
        } catch (bulkUnavailableError: unknown) {
          logger.error(
            { err: bulkUnavailableError, ...logContext },
            `[SyncService] Error during bulk marking members as unavailable.`,
          );
      ...
      logger.info(
        logContext,
        `[SyncService] Successfully finished guild_members table sync. Committing transaction.`,
      );
    });
  } catch (error: unknown) {
    logger.error(
      { err: error, ...logContext },
      `[SyncService] Transaction failed during guild members table sync. Changes rolled back.`,
    );
    throw new Error(
      `Error syncing guild members table for guild ${guildId}: ${
