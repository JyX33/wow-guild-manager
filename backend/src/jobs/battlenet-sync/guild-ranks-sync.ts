...
export async function syncGuildRanks(
  rankModel: RankModel,
  guildId: number,
  ...
  try {
    ...
    logger.debug({
      ...logContext,
      rankIds: Array.from(rosterRankIds),
      counts: rankCounts,
    }, `[SyncService] Calculated rank counts from BNet roster.`);

    const existingRanks = await rankModel.getGuildRanks(guildId);
    const existingRankMap = new Map(
      existingRanks.map((r: GuildRank) => [r.rank_id, r]),
    );
    logger.debug(
      { ...logContext, count: existingRankMap.size },
      `[SyncService] Fetched ${existingRankMap.size} existing ranks from DB.`,
    );
    ...
    for (const rankId of rosterRankIds) {
      ...
      if (!rankRecord) {
        ...
        logger.info(
          rankLogContext,
          `[SyncService] Rank ${rankId} not found locally. Ensuring rank exists with default name "${defaultName}" and setting count to ${currentCount}.`,
        );
        updatePromises.push(
        ...
        if (rankRecord.member_count !== currentCount) {
          logger.debug(
            rankLogContext,
            `[SyncService] Updating member count for rank ${rankId} from ${rankRecord.member_count} to ${currentCount}.`,
          );
          updatePromises.push(
            rankModel.updateMemberCount(guildId, rankId, currentCount)
              .catch((err: unknown) =>
          ...
          logger.trace(
            rankLogContext,
            `[SyncService] Rank ${rankId} member count (${currentCount}) is unchanged.`,
          );
        }
    ...
    for (const [rankId, rankRecord] of existingRankMap.entries()) {
      if (rankRecord.member_count !== 0) {
        ...
        logger.info(
          rankLogContext,
          `[SyncService] Rank ${rankId} ("${rankRecord.rank_name}") no longer in BNet roster. Setting member count to 0.`,
        );
        updatePromises.push(
    ...
    results.forEach((result, index) => {
      if (result.status === "rejected") {
        logger.warn(
          ...
          `[SyncService] A rank update/creation promise failed (already logged).`,
        );
      }
    });
    ...
    logger.error(
      ...
      `[SyncService] Critical error during rank sync setup.`,
    );
