  ...
  try {
    ...
    try {
      ...
      for (const member of bnetGuildRoster.members) {
        ...
        const dbCharacter = await findOrCreateCharacterForRosterMember(dependencies, member);

        // Check if the character needs syncing based on last_synced_at
        if (!dbCharacter.last_synced_at) {
          logger.debug({ charId: dbCharacter.id, charName: dbCharacter.name }, '[BattleNetSync] Character never synced, adding to sync list.');
          charactersToSync.push(dbCharacter);
        } else {
          const lastSynced = DateTime.fromJSDate(new Date(dbCharacter.last_synced_at)); // Ensure it's a Date object first
          if (lastSynced < eightHoursAgo) {
            logger.debug({ charId: dbCharacter.id, charName: dbCharacter.name, lastSynced: dbCharacter.last_synced_at }, '[BattleNetSync] Character outdated, adding to sync list.');
            charactersToSync.push(dbCharacter);
          } else {
            logger.debug({ charId: dbCharacter.id, charName: dbCharacter.name, lastSynced: dbCharacter.last_synced_at }, '[BattleNetSync] Character recently synced, skipping.');
          }
        }
      }

      logger.info(
        { guildId: guild.id, count: charactersToSync.length, totalRoster: bnetGuildRoster.members.length },
      ...
      const characterSyncPromises = charactersToSync.map(character =>
        characterSyncLimit(() => // Wrap the async function call
          syncCharacter(
...
async function findOrCreateCharacterForRosterMember(
  ...
  logger.info({ charName: name, realm: realmSlug }, '[BattleNetSync] Character not found in DB, creating minimal record.');
  ...
  const createdCharacter = await characterModel.createCharacter(newCharacterData);

  // Add a check in case createCharacter unexpectedly returns null/undefined
  if (!createdCharacter) {
      throw new Error(`Failed to create minimal character record for ${name}-${realmSlug}`);
  }

  logger.info({ charId: createdCharacter.id, charName: name, realm: realmSlug }, '[BattleNetSync] Created new minimal character record.');
...
export async function runSync(dependencies: SyncDependencies): Promise<void> {
  ...
  try {
    ...
    logger.info({ count: guilds.length }, '[BattleNetSync] Starting orchestrated sync for outdated guilds');
    ...
    const syncPromises = guilds.map((guild) =>
      limit(async () => { // Wrap the async operation in the limiter
        ...
        try {
          ...
          await dependencies.guildModel.update(guild.id, { last_roster_sync: new Date().toISOString() });
          logger.info({ guildId: guild.id }, '[BattleNetSync] Updated guild last_synced_at timestamp.');
          ...
          try {
             await dependencies.guildModel.update(guild.id, { last_roster_sync: new Date().toISOString() });
             logger.warn({ guildId: guild.id }, '[BattleNetSync] Updated guild last_synced_at timestamp after unexpected error.');
          } catch (updateErr) {
             logger.error({ err: updateErr, guildId: guild.id }, '[BattleNetSync] Failed to update guild last_synced_at after unexpected error.');
          }
        }
      })
    );
