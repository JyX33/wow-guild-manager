...
export async function migrateSubscriptionCharacters(): Promise<void> {
  logger.info('Starting character migration from event subscriptions...');
  ...
  try {
    await withTransaction(async (client: Pool) => {
      ...
      logger.info(`Found ${uniqueCharacters.length} unique characters to migrate`);
      ...
      for (const char of uniqueCharacters) {
        ...
        if (char.realm) {
          ...
          try {
            ...
            logger.warn({ err, charName: char.character_name }, `Could not determine realm for character ${char.character_name}, using 'unknown'`);
          }
        }
        ...
        if (existingCharResult.rows.length > 0) {
          logger.info({ charName: char.character_name, userId: char.user_id }, `Character ${char.character_name} already exists for user ${char.user_id}, skipping`);
          continue;
        }
        ...
        logger.info({ characterId, charName: char.character_name, userId: char.user_id }, `Created character #${characterId}: ${char.character_name} for user ${char.user_id}`);
      }
      ...
      for (const userRow of usersResult.rows) {
        ...
        if (recentCharResult.rows.length > 0) {
          ...
          logger.info({ mainCharId, userId }, `Set character #${mainCharId} as main for user ${userId}`);
        }
      }
    ...
    logger.error({ err: error }, 'Character migration failed:');
    throw error;
  }
}
...
export async function updateSubscriptionsToUseCharacterId(): Promise<void> {
  logger.info('Updating event subscriptions to use character_id...');
  ...
  try {
    await withTransaction(async (client: Pool) => {
      ...
      logger.info(`Found ${subscriptions.length} subscriptions to update`);
      ...
      for (const sub of subscriptions) {
        ...
        logger.info({ subId: sub.id, charId: sub.character_id }, `Updated subscription #${sub.id} to use character #${sub.character_id}`);
      }

      logger.info('Event subscription update completed successfully');
    });
  } catch (error) {
    logger.error({ err: error }, 'Event subscription update failed:');
    throw error;
  }
}
...
export async function runFullMigration(): Promise<void> {
  try {
    logger.info('Starting full character migration process...');
    ...
    logger.info('Full character migration completed successfully');
  } catch (error) {
    logger.error({ err: error }, 'Full character migration failed:');
    throw error;
  }
}
...
if (require.main === module) {
  runFullMigration()
    .then(() => process.exit(0))
    .catch(err => {
      logger.error({ err }, 'Migration failed:'); // Use logger here too
      process.exit(1);
    });
}