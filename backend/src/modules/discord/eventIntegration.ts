...
export async function postEventToDiscord(event: Event): Promise<void> {
    ...
    if (!channelId) {
        logger.error('DISCORD_EVENT_CHANNEL_ID environment variable not set. Cannot post event.');
        return;
    }
    ...
    try {
        ...
        if (!(channel instanceof TextChannel)) {
            logger.error(`Configured Discord channel (${channelId}) is not a text channel.`);
            return;
        }
        ...
        logger.info(`Created thread ${thread.id} for event ${event.id} in channel ${channelId}`);
            ...
            .addFields(
                {
                    ...
                    value: event.start_time
                        ? new Date(event.start_time).toLocaleString('en-US', { dateStyle: 'full', timeStyle: 'short' })
                        : 'Not specified',
                },
                {
                    name: 'Location',
        ...
        logger.error({ err: error, eventId: event.id }, 'Failed to post event to Discord or update database.');
