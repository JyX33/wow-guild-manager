...
export class CharacterClassificationService {
   ...
   */
  async getClassifiedGuildMembers(guildId: number): Promise<ClassifiedMember[]> {
    logger.info({ guildId }, `[ClassificationService] Starting classification for guild ID: ${guildId}`);
    try {
      ...
      const members: (DbGuildMember & { character: DbCharacter })[] = result.rows;

      if (members.length === 0) {
        logger.info({ guildId }, `[ClassificationService] No members found for guild ID: ${guildId}`);
        return [];
      }

      // 2. Separate Known/Unknown
      const knownMembersMap = new Map<number, (DbGuildMember & { character: DbCharacter })[]>();
      ...
      const hashableUnknownsMap = new Map<string, (DbGuildMember & { character: DbCharacter })[]>();
      ...
      for (const member of unknownMembers) {
        if (member.character.toy_hash) {
          const hashGroup = hashableUnknownsMap.get(member.character.toy_hash) || [];
      ...
      for (const hashGroup of hashableUnknownsMap.values()) {
        classifiedResults.push(...this._classifyUnknownHashableGroup(hashGroup));
      }
      ...
      logger.info({ guildId, count: classifiedResults.length }, `[ClassificationService] Finished classification for guild ID: ${guildId}. Classified ${classifiedResults.length} members.`);
      ...
      logger.error({ err: error, guildId }, `[ClassificationService] Error classifying members for guild ${guildId}`);
      throw new AppError(`Error classifying guild members: ${error instanceof Error ? error.message : String(error)}`, 500);
    }
  }

  // --- Helper methods for classification logic ---

  private _classifyKnownUserGroup(members: (DbGuildMember & { character: DbCharacter })[]): ClassifiedMember[] {
    ...
    if (!userId) {
        logger.warn(`[ClassificationService] _classifyKnownUserGroup called with members lacking user_id.`);
  ...
  private _classifyUnknownHashableGroup(members: (DbGuildMember & { character: DbCharacter })[]): ClassifiedMember[] {
    if (!members || members.length === 0) {
      return [];
    }
    ...
    if (!groupKey) {
        logger.warn(`[ClassificationService] _classifyUnknownHashableGroup called with members lacking toy_hash.`);
  ...
  private _classifyUnhashableMember(member: (DbGuildMember & { character: DbCharacter })): ClassifiedMember {
