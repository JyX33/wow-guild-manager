...
export class OnboardingService {
  private battleNetApiClient: BattleNetApiClientEnhanced;

  // Inject dependencies - accepts enhanced client type
  constructor(apiClient: BattleNetApiClientEnhanced) {
  ...
  private async _prepareOnboardingCharacterUpdatePayload(
    ...
    if (bnet_guild_id) {
      try {
        const localGuild = await guildModel.findOne({
          bnet_guild_id: bnet_guild_id,
        });
        if (localGuild) {
          determinedRegion = localGuild.region;
          logger.debug(
            { charId: character.id, guildId: localGuild.id },
            "[OnboardingService] Linked character to existing local guild during enhanced sync.",
          );
        } else {
          logger.warn(
            { charId: character.id, bnetGuildId: bnet_guild_id },
            "[OnboardingService] Local guild not found for character during enhanced sync. Guild link skipped for now.",
          );
        }
      } catch (guildError) {
        logger.error(
          { err: guildError, charId: character.id, bnetGuildId: bnet_guild_id },
          "[OnboardingService] Error fetching local guild during enhanced sync.",
        );
      }
    }

    // Ensure region is valid before assigning
    const validRegion = determinedRegion &&
        ["us", "eu", "kr", "tw", "cn"].includes(determinedRegion)
  ...
  async processNewUser(
    ...
    logger.info(
      { userId, region },
      "[OnboardingService] Starting onboarding process...",
    );

    try { // Main try block for the whole process
      // 1. Fetch WoW Profile
      logger.debug({ userId }, "[OnboardingService] Fetching WoW profile...");
      const wowProfile = await this.battleNetApiClient.getWowProfile(
        region,
        accessToken,
      );
      const wowAccounts = wowProfile.wow_accounts || [];
      logger.debug(
        { userId, accountCount: wowAccounts.length },
        "[OnboardingService] WoW profile fetched.",
      );

      // 2. Sync Characters (Creates/Updates character records in DB)
      logger.debug(
        { userId },
        "[OnboardingService] Syncing characters from profile...",
      );
      ...
      logger.debug(
        { userId, ...syncResult },
        "[OnboardingService] Initial character sync from profile completed.",
      );

      // --- Start: Fetch Enhanced Data for Synced Characters ---
      if (syncResult.processedIds && syncResult.processedIds.length > 0) {
        logger.info(
          { userId, count: syncResult.processedIds.length },
          "[OnboardingService] Starting enhanced data sync for processed characters...",
        );
        const charactersToEnhance = await characterModel.findByIds(
          syncResult.processedIds,
        );

        for (const character of charactersToEnhance) {
          if (!character.name || !character.realm || !character.region) {
            logger.warn(
              { userId, charId: character.id },
              "[OnboardingService] Skipping enhanced sync for character due to missing name, realm, or region.",
            );
            continue;
          }
          try {
            ...
            }, "[OnboardingService] Fetching enhanced data...");
            const enhancedData = await this.battleNetApiClient
              .getEnhancedCharacterData(
                character.realm,
            ...
            if (enhancedData) {
              ...
              logger.debug(
                { userId, charId: character.id, charName: character.name },
                "[OnboardingService] Updating character with enhanced data...",
              );
              ...
              logger.info({
                ...
                charId: character.id,
                charName: character.name,
              }, "[OnboardingService] Enhanced data sync successful.");
            } else {
              logger.info(
                { userId, charId: character.id, charName: character.name },
                "[OnboardingService] No enhanced data found (e.g., 404), skipping update.",
              );
            }
            ...
            logger.error(
              ...
              "[OnboardingService] Error during enhanced data sync for character.",
            );
          }
        } // End loop through charactersToEnhance
        logger.info(
          { userId },
          "[OnboardingService] Enhanced data sync loop finished.",
        );
      }
      ...
      logger.debug(
        { userId },
        "[OnboardingService] Re-fetching characters from DB after enhanced sync...",
      );
      const dbCharacters: DbCharacter[] = await characterModel.findByUserId(
        userId,
      );
      logger.debug(
        { userId, count: dbCharacters.length },
        "[OnboardingService] Fetched characters from DB.",
      );
      ...
      for (const char of dbCharacters) {
        const guildInfoFromProfile = (char as any).profile_json
          ?.guild as BattleNetCharacter["guild"];
        const charBnetId = (char as any).profile_json?.id;

        if (
          guildInfoFromProfile?.id && guildInfoFromProfile.name &&
          guildInfoFromProfile.realm?.slug && charBnetId
        ) {
          if (!uniqueGuilds.has(guildInfoFromProfile.id)) {
            ...
            if (
              !guildRegion && char.region &&
              ["us", "eu", "kr", "tw", "cn"].includes(char.region)
            ) {
            ...
            if (
              guildRegion &&
              ["us", "eu", "kr", "tw", "cn"].includes(guildRegion)
            ) {
              ...
              logger.warn(
                {
                  userId,
                  charName: char.name,
                  guildName: guildInfoFromProfile.name,
                  realmSlug,
                  charRegion: char.region,
                  loginRegion: region,
                },
                "[OnboardingService] Could not determine a valid region for guild found in character profile.",
              );
            }
          }
        }
      ...
      logger.info(
        { userId, count: uniqueGuilds.size },
        "[OnboardingService] Extracted unique guilds from character profiles.",
      );
      ...
      for (const guildInfo of uniqueGuilds.values()) {
        ...
        try { // Try block for processing a single guild
          ...
          localGuild = await guildModel.findOne({
            bnet_guild_id: guildInfo.bnet_guild_id,
          });
          if (!localGuild) {
            localGuild = await guildModel.findByNameRealmRegion(
              guildInfo.name,
              guildInfo.realmSlug,
              guildInfo.region,
            );
          ...
          if (!localGuild) {
            logger.info(
              { guildName: guildInfo.name, realm: guildInfo.realmSlug },
              "[OnboardingService] Creating minimal guild record during onboarding.",
            );
          ...
          logger.debug(
            { userId, guildName: guildInfo.name },
            "[OnboardingService] Fetching roster...",
          );
          const roster: BattleNetGuildRoster = await this.battleNetApiClient
            .getGuildRoster(
              guildInfo.region,
              guildInfo.realmSlug,
          ...
          for (const member of roster.members) {
            if (member.rank === 0) {
              ...
              logger.debug({
                ...
                gmCharName: gmCharacterName,
                gmBnetId,
              }, "[OnboardingService] Found potential GM in roster.");
              break;
            }
          }
          ...
          if (gmBnetId && localGuild) {
            try { // Try block for looking up local GM by bnet_id
              const localGmCharacter = await characterModel.findOne({
                bnet_character_id: gmBnetId,
              });
              ...
              if (localGmCharacter && localGmCharacter.user_id) {
                ...
                logger.debug(
                  {
                    ...
                    leaderUserId: localGmCharacter.user_id,
                  },
                  "[OnboardingService] Found local GM character. Updating guild leader_id with user ID...",
                );
                ...
                logger.info(
                  ...
                  "[OnboardingService] Successfully updated guild leader ID (user ID).",
                );
                ...
                logger.error(
                  {
                    userId,
                    guildId: localGuild.id,
                    gmLocalId: localGmCharacter.id,
                    gmBnetId,
                  },
                  "[OnboardingService] Found local GM character but user_id is missing. Cannot set leader_id.",
                );
                ...
                logger.warn(
                  ...
                  "[OnboardingService] GM found in roster, but no corresponding local character record found by bnet_id. Leader ID not set.",
                );
                // Update only sync time
                try {
                  await guildModel.update(localGuild.id, {
              ...
              logger.error(
                ...
                "[OnboardingService] Database error looking up local GM by bnet_id.",
              );
            ...
            logger.info(
              { userId, guildName: guildInfo.name, guildId: localGuild.id },
              "[OnboardingService] No rank 0 member found in roster or GM BNet ID missing. Updating sync time only.",
            );
            try {
              await guildModel.update(localGuild.id, {
                last_roster_sync: new Date().toISOString(),
            ...
            logger.warn(
              {
                userId,
                guildName: guildInfo.name,
                bnetGuildId: guildInfo.bnet_guild_id,
              },
              "[OnboardingService] Cannot update sync time as local guild record does not exist.",
            );
          ...
          logger.error(
            ...
            "[OnboardingService] Error fetching or processing roster for guild leader identification.",
          );
          const statusCode =
            typeof rosterError === "object" && rosterError !== null
              ? ("status" in rosterError
                ? (rosterError as { status?: number }).status
                : undefined) ||
                ("response" in rosterError &&
                    typeof (rosterError as { response?: unknown }).response ===
                      "object" &&
                    (rosterError as { response?: unknown }).response !== null &&
                    "status" in
                      ((rosterError as { response?: object })
                        .response as object)
                  ? ((rosterError as { response: { status?: number } }).response
                    .status)
                  : undefined)
              : undefined;
          if (statusCode === 404) {
            logger.warn(
              { userId, guildName: guildInfo.name },
              "[OnboardingService] Roster not found (404) for guild during leader identification.",
            );
          }
      ...
      logger.debug(
        { userId },
        "[OnboardingService] Updating user sync timestamp.",
      );
      await userModel.updateCharacterSyncTimestamp(userId);

      logger.info(
        { userId },
        "[OnboardingService] Onboarding process completed successfully.",
      );
    } catch (error: unknown) { // Catch for the main processNewUser try block
      logger.error(
        { err: error, userId },
        "[OnboardingService] Error during onboarding process:",
      );
